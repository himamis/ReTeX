...
public class TeXFontParserD implements TeXFontParser {
    ...
    private static class ExtensionParser implements CharChildParser {
        ...
        public void parse(Element el, char ch, FontInfo info) throws ResourceParseException {
            ...
            extensionChars[DefaultTeXFont.REP] = TeXFontParserD
		.getIntAndCheck("rep", el);
            // get optional integer attributes
            extensionChars[DefaultTeXFont.TOP] = TeXFontParserD
		.getOptionalInt("top", el, DefaultTeXFont.NONE);
            extensionChars[DefaultTeXFont.MID] = TeXFontParserD
		.getOptionalInt("mid", el, DefaultTeXFont.NONE);
            extensionChars[DefaultTeXFont.BOT] = TeXFontParserD
    ...
    private static class KernParser implements CharChildParser {
        ...
        public void parse(Element el, char ch, FontInfo info) throws ResourceParseException {
            ...
            int code = TeXFontParserD.getIntAndCheck("code", el);
            // get required float attribute
            float kernAmount = TeXFontParserD.getFloatAndCheck("val", el);
    ...
    private static class LigParser implements CharChildParser {
        ...
        public void parse(Element el, char ch, FontInfo info) throws ResourceParseException {
            // get required integer attributes
            int code = TeXFontParserD.getIntAndCheck("code", el);
            int ligCode = TeXFontParserD.getIntAndCheck("ligCode", el);
    ...
    private static class NextLargerParser implements CharChildParser {
        ...
        public void parse(Element el, char ch, FontInfo info) throws ResourceParseException {
            // get required integer attributes
            String fontId = TeXFontParserD.getAttrValueAndCheckIfNotNull("fontId", el);
            int code = TeXFontParserD.getIntAndCheck("code", el);
    ...
    public TeXFontParserD() throws ResourceParseException {
	this(ClassLoader.getSystemResourceAsStream(RESOURCE_NAME), RESOURCE_NAME);
    }
    
    public TeXFontParserD(String name) {
    	this(ClassLoader.getSystemResourceAsStream(name), name);
    }
    
    public TeXFontParserD(InputStream file, String name) throws ResourceParseException {
    ...
    public TeXFontParserD(Object base , InputStream file, String name) throws ResourceParseException {
	this.base = base;
	factory.setIgnoringElementContentWhitespace(true);
	factory.setIgnoringComments(true);
	try {
	...
	Element fontDescriptions = (Element)root.getElementsByTagName("FontDescriptions").item(0);
		...
		String include = getAttrValueAndCheckIfNotNull("include", (Element)list.item(i));
		    fi = parseFontDescriptions(fi, ClassLoader.getSystemResourceAsStream(include), include);
    ...
    public Map<String,CharFont> parseSymbolMappings() throws ResourceParseException {
		...
		Element map;
		try {
			map = factory.newDocumentBuilder().parse(ClassLoader.getSystemResourceAsStream(include)).getDocumentElement();
		} catch (Exception e) {
