...
public class MakeIcon {
	...
	public static void texIcons() {
		...
		Icon icon = IconHelper.createIcon(formula.new TeXIconBuilder()
				.setStyle(TeXConstants.STYLE_DISPLAY).setSize(25).build());
	...
	public static void symbolIcons() {
		...
		for (int i = 0; i < symbols.length; i++) {
			...
			BufferedImage image = new BufferedImage(icon.getIconWidth(),
					icon.getIconHeight(), BufferedImage.TYPE_INT_ARGB);
			Graphics2D g2 = image.createGraphics();
			IconHelper.createIcon(icon).paintIcon(new JLabel(), g2, 0, 0);
